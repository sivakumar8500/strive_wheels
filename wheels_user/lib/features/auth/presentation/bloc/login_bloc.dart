...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  LoginBloc({required this.loginWithPhoneUseCase}) : super(const LoginInitial()) {
  ...
  Future<void> _onLoginSubmitted(
    ...
    if (event.phoneNumber.isEmpty || event.phoneNumber.length < 10) {
      emit(const LoginFailure(errorMessage: 'Please enter a valid mobile number'));
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        emit(const LoginFailure(errorMessage: 'Login failed. Please try again.'));
      }
    } catch (e) {
