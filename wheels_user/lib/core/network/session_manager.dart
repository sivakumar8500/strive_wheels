...
/// Centralized Session & Token Refresh Manager for Customer App.
...
class SessionManager {
  ...
  SessionManager(this._prefs, {Dio? dio}) : _dio = dio ?? Dio();

  String? get accessToken =>
      _prefs.getString('access_token') ?? _prefs.getString('user_token');

  String? get refreshToken =>
      _prefs.getString('refresh_token') ?? _prefs.getString('user_refresh_token');
  ...
  Future<String?> getValidAccessToken() async {
    if (isTokenExpiringSoon()) {
      debugPrint('[SessionManager] Access token expiring soon or missing. Proactively refreshing...');
  ...
  Future<String?> refreshSession({bool force = false}) async {
    ...
    if (_inFlightRefreshFuture != null) {
      debugPrint('[SessionManager] In-flight refresh in progress. Awaiting result...');
  ...
  Future<String?> _performRefresh() async {
    ...
    if (curRefreshToken == null || curRefreshToken.isEmpty) {
      debugPrint('[SessionManager] No refresh token found. Triggering session expiry.');
    ...
    try {
      ...
      debugPrint('[SessionManager] Sending refresh request to $refreshPath...');
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (newAccess != null && newAccess.isNotEmpty) {
          debugPrint('[SessionManager] Refresh success! Issued new access token.');
      ...
      if (response.statusCode == 400 || response.statusCode == 401 || response.statusCode == 422) {
        debugPrint('[SessionManager] Refresh token rejected with HTTP ${response.statusCode}. Expiry triggered.');
        await handleSessionExpiry();
        return null;
      }
      ...
      if (status == 400 || status == 401 || status == 403 || status == 422) {
        debugPrint('[SessionManager] Refresh token error ($status). Triggering session expiry.');
        await handleSessionExpiry();
        return null;
      }
      debugPrint('[SessionManager] Network or server error during refresh: $e. Session retained.');
      return null;
    } catch (e) {
      debugPrint('[SessionManager] Unexpected exception during refresh: $e');
      return null;
    }
  ...
  Future<void> handleSessionExpiry() async {
    debugPrint('[SessionManager] Session expired. Clearing credentials & notifying callback.');
    await clearSession();
